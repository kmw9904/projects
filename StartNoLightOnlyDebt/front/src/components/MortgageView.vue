...
const mergedProducts = computed(() => {
  const baseList = products.value.mortgage.result.baseList || [];
  const optionList = products.value.mortgage.result.optionList || [];
...
// store.mortgages가 업데이트되면 products.value에 데이터를 반영
watch(
  () => store.mortgages,
  (newValue) => {
    if (newValue) {
